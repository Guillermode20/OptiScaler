...
- The input-predicted timewarp machinery (`ReprojInputPredictor`, KCD2 phase model, `InputPredictorResponse`) was
  ...
  silently sees zero deltas. Sync-path limitation: the game thread is blocked inside Present during warp bursts, so
  WM_INPUT is not pumped mid-burst; async (presenter thread) sees fully fresh totals. Grid-anchored sampling
