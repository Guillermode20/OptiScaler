...
- Async-timewarp-on-KCD2 status: no async-attempt log exists for the current build. Prior d3d12core crashes correlated with coercing KCD2's 2-buffer swapchain to 3 during resize; async legitimately needs 3 buffers at creation, so the next `Reproj.Async=true` run either logs a graceful fallback (`waitable main swapchain unavailable` / `virtualization unavailable`) or reproduces the crash — either outcome pins the cause. KCD2 ships with `Async=true` locally for that test.
