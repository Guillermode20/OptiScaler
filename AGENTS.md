...
2. Each game present publishes an anchor packet: the capture worker submits a full-resolution copy of the HUD-less world gated on the mid-frame world fence when one fired (`_worldFence`, signaled by the ResTrack CL-submit hook the moment the CL containing the KCD2 world snapshot is submitted; per-pass renderers signal mid-frame, single-CL-per-frame renderers at present-time submission — safe either way, with a present-time game-thread fallback signal so a never-submitted CL cannot stall the capture queue), then a copy of the isolated UI gated on the present-time input fence. The warp gate is the color copy; the UI trails and the presenter composites the newest completed UI (borrowing the held previous anchor's UI for the first slot of a new anchor — `uiBorrow=`). No per-anchor queue pin. Depth copies are skipped unless `ReprojDepthEnabled` is on (default off).
3. The presenter fills every display slot with exactly one `Present(1, 0)` of the newest anchor whose capture fence has **already completed**; otherwise it re-warps its active anchor (counted as `capWait`). Warp count is whatever the display needs — never bounded by a warp cap — and pose age never disables warping (ATW over a stalled renderer is the point). An occluded `Present` (`DXGI_STATUS_OCCLUDED`, alt-tab/minimize) is invisible-but-successful: the presenter backs off to ~20 Hz sleeps, resets its deadline grid, and counts no missed slots, so tabbing out and back in causes no burst.
4. Warps are rotation-only camera timewarp on a dedicated normal-priority COMPUTE queue (VKD3D serializes DIRECT queues ~10–17 ms behind the game, so the warp must not run there). The presenter thread waits for that short compute submission before calling `Present`; never enqueue a per-slot compute-fence wait on the game's DIRECT queue, because every later game command then serializes behind the 120 Hz presenter. The isolated HUD is composited unwarped in the same dispatch by RPD — exactly once per output. Unwarped blits stay on the DIRECT SC queue (copy + raster UI in one submit, no cross-queue round-trip).
5. A compute-fence late latch rewrites the warp constants just before the present deadline from fresh raw mouse motion; the sample lead is **adaptive by default** (`LateSampleLead=auto` → 0): after each warp completes the presenter measures its headroom to the deadline and slides the next slot's sample later (±0.25 ms, clamped `[2.0, 6.0]`) down to the reduce edge at ~2 ms of headroom — sampling as late as the signal+warp+copy cost plus CPU-wake/Present margin allows, instead of a fixed constant. KCD2 camera snapshots include the raw-mouse totals sampled in the same callback, so residual input is measured directly from the authoritative pose baseline. With no fresh input it falls back to rendered-camera angular-velocity extrapolation.
...
| `[AsyncTimewarp] LateSampleLead` | 0 (= auto) | ms before the present deadline to sample late-latch mouse input. `auto`/`0` = **adaptive**: hunts the sample as late as the warp allows (see pipeline step 5); a fixed value `>0.5` overrides with the old constant lead. `sampLead=` in the log shows the effective lead |
| `[AsyncTimewarp] RepeatWarp` | true | Warp on repeated display slots. `true`/auto is now **adaptive**: repeated slots take the cheap blit path (no warp) while the source cannot sustain its frame-rate cap or the game thread stalls behind the GPU (`EvaluateRepeatWarpShed` in `AReprojPresenter.cpp`), then full warps resume when the source holds the cap again. `false` = never warp repeats (saves the most GPU time, display content cadence drops to the source rate) |
| `[FrameGen] DrawUIOverFG` | false | Required for HUD composition (with Hudfix) |

Hardcoded constants agents must know: warp timestep clamp `2.5`; adaptive late-sample lead hunts ±0.25 ms/slot (`SAMPLE_LEAD_STEP_MS`) within `[2.0, 6.0]` ms (`SAMPLE_LEAD_MIN/MAX_MS`) from post-warp headroom bands: reduce the lead when headroom `> 2.0` ms (`SAMPLE_LEAD_REDUCE_HEADROOM_MS`, sample released too early), grow when `< 0.9` ms (`SAMPLE_LEAD_GROW_HEADROOM_MS`, crowding the vblank); controller state `_lateSampleLeadMs` in `AReproj_Dx12.cpp` `DispatchPacketWarp`; per-slot late rotation ceiling `0.11 rad` (~6.3°); hitch hold after ~2.5 source periods without a publish; dispatch lead adapts from post-wait headroom within `[3.0, min(8, 0.75·period)]` ms; presenter thread runs at `THREAD_PRIORITY_TIME_CRITICAL`. Adaptive repeat-warp shed constants (`AReprojPresenter.cpp` `EvaluateRepeatWarpShed`): cadence engage/release ratios `1.02x`/`1.00x` of the configured source-cap period, or a hardcoded 60 Hz floor when the source is uncapped (shedding engages below ~58.8 FPS and restores only after recovery to 60); stall (game-present block, pacing sleep excluded) engage/release `8`/`6` ms via a time-decayed EMA; minimum shed dwell `400` ms. Tune from live `shed=`/`stallEma=` log output — do not re-add menu controls.

### Reading the once-per-second log line

`Reproj: source=… display=… (new=… repeat=…) missed=… interval=mean/p95 lead=… sampLead=… poseAge=… queue=… late=applied/sampled maxDeg=… hud=… dropAnchor=… capC=… capWait=… uiBorrow=… latch=lateCam/packetBase/fallback lateAge=…ms sensX=… hold=… shed=… stallEma=…ms (async virtual swapchain|safe sync, block=… pace=…)`.
Healthy at 60 Hz source / 120 Hz display: source 59–60, display ≥ 117, `missed` < 2/s after warm-up, roughly equal `new`/`repeat`, `dropAnchor=0`, low `block` (`pace` is the intentional cap sleep, reported separately), `late` applied and nonzero during motion, `hud` tracking displayed outputs. `capC` = DIRECT captures; `capWait` = slots that reused the active anchor because the newest capture was unfinished (nonzero is fine as long as the warp queue never stalls). `uiBorrow` = new-anchor slots that composited the held previous anchor's UI while the new anchor's own UI copy trailed (expected ≈ capWait: the borrow is what lets the anchor display one slot earlier); a healthy session shows it shrinking toward the number of captures that barely missed the slot boundary. `poseAge` dropping while `capWait`/`uiBorrow` stay low means the mid-frame world fence is landing: the anchor's color copy starts while the game still finishes its frame, so the first display moves one slot earlier. `sampLead=` is the effective late-latch sample lead (adaptive by default; converges toward warp duration + margin, smaller = fresher mouse input). `latch=` splits applied steering into late-camera-pose / packet-baseline / velocity-fallback slots; `lateAge` is the mean age of the late pose at use; `sensX` is the auto-tracked radians/count (sanity: stable, ~1–3e-4). `hold` counts slots frozen at timeStep 0 during a publish stall (see Hitch hold below); steady-state it must be 0. `shed=1` means the adaptive repeat-warp shed is active (repeated slots are blitted, not warped) because the source was not holding its cap or the game thread was stalling; `stallEma=` is the smoothed game-present block feeding that decision. Healthy steady state: `shed=0`.

### UI borrow hold (latency pass, v10.0.1.37+)

The new anchor's UI copy trails its color copy, so the first display of a new anchor composites the held previous anchor's UI (`uiBorrow=`); 16 ms stale is invisible on HUD elements. `_heldPacketIndex` stays `Presenting` until the NEXT new-anchor switch or loop exit, never per-slot (early exits must retire it) — it exists ONLY for that UI borrow. The swap-smooth color blend that originally shared this hold (v37 `PrevColor` t3 SRV) was REMOVED in v40: it sampled the previous anchor's image with the CURRENT anchor's baked homography, which misaligns the previous frame by the full inter-anchor rotation whenever the camera moves — a visible ghost/double during look-around. Do not re-add it without a second per-anchor output-pixel→source-UV bake computed from the previous anchor's own captured pose to the current display pose.

### Invariants (do not break)

- The game thread never waits for the GPU on the capture path: packet exhaustion AND a busy capture allocator both drop the anchor (signal, submit, advance, count `dropAnchor`) instead of stalling. Virtual-buffer handoff remains bounded (up to 5 s) only when a capture actually reads that virtual resource. **KCD2 isolated HUD-less/UI captures never read it:** `CaptureFramePacket()` clears both `packet.handoffFence` fields and the present caller must forward those null/zero fields unchanged to `SubmitReprojectionBuffer()`—never fall back to `completionFence`, which reintroduces the 2-buffer CPU wait. A skipped anchor is invisible; a stalled game thread deepens every hitch behind it.
...
- Pacing is gated by BOTH the DXGI frame-latency waitable AND a software deadline of `lastPresent + refreshPeriod` (on Proton the waitable alone fires on queue capacity, ~3–4 ms with tearing, and must not drive cadence). The presenter runs on a completion clock: the next deadline derives from Present-completion timestamps and DXGI frame-statistics phase correction is bypassed entirely, because Wine advances those statistics per presented output rather than per physical vblank. The dispatch lead adapts symmetrically (+0.25/−0.1 ms) within `[3.0, min(8, 0.75·period)]` ms so it cannot ratchet to the cap.
...
- Capture prefers a dedicated COPY queue (DMA, no shader-core contention), falling back to COMPUTE then game DIRECT. Do not route unwarped blits through COMPUTE: the copy + raster UI live together on DIRECT in a single submit.
- Do not retry: CPU-blocking queue-arrival latching (cadence regressed ~2% → 7% drops) or a high-priority presenter queue (starved source rendering to ~35 FPS). Do not make virtual-buffer handoff non-blocking after capture submission: the game must not reuse the still-captured virtual resource.
...
- Warp mode is hardcoded rotation-only. Translation (walking/hills) is a known limitation: the warp assumes infinite depth, so near geometry parallaxes wrong on stale anchors — no small fix exists; depth-corrected warping was prototyped (pre24/25) and fully removed after regressing feel without a footage-tuning channel. Do not re-add without one.
...
- HUD isolation (`Kcd2Scaleform` + `Kcd2HudIsolation`, live-validated): KCD2 renders Scaleform UI into the active backbuffer just before `Present()`. `BeginDisplay`/`EndDisplay` hooks snapshot the clean 3D world into `hudlessTexture`, clear `uiTexture`, and redirect Scaleform's RTV there. Isolation resources use a fenced generation ring and cannot be reused until their packet COPY completes; if every generation is busy, the frame fails closed to the composed game image. The synchronous fallback reuses the same split (`CopyLastFrame` → `_lastColor`, UI → `_uiColor`, composite after warp) so it also yields exactly one unwarped HUD.
...
Detailed per-slot telemetry (`ReprojTelemetry v=1`, slot dumps, `cause.*`/`step.*` fields) was removed 2026-09-02. The once-per-second `Reproj:` line above is the primary health instrument — keep its keys stable and do not add per-slot allocation/log/fence-wait/readback. `ReprojPipe v=1` (added for the RX 6600 XT source-dip investigation) is a separate, parser-stable **250 ms aggregate** line: flat `key=value` fields cover game publication CPU phases, actual `AdvanceReprojectionBuffer` fence waits, capture-worker backlog/CPU time, isolation/fallback path counts, READY fence polls, and warp/blit output topology. It uses relaxed atomic counters only and is emitted by the presenter thread (never game `Present()`, where synchronous file logging would perturb source timing); do not turn it into an event trace or GPU timestamp/readback system. `tests/reprojection/analyze_telemetry.py` and its fixture still parse the dead v=1 format, so they are stale until rewritten; the inert one-slot bookkeeping shim in the presenter exists only until its call sites are cleaned up.
