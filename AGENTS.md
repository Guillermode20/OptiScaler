...
- Scope cut (2026-08-31): `HybridFsrGenerator`, `Kcd2Input`, `ReprojInputPredictor`, and `TargetPoseResolver` were
  ...
  `Kcd2Camera` pose) -> per-vblank warp of the newest anchor -> present, with a raw-mouse late latch at scanout as the
  only 120 Hz steering source (`ApplyLateInput`, `[Reproj] LateLatch` / `LateLatchFence`, `MouseSensitivityX/Y`).
  HUD isolation is unchanged and must never be removed. `docs/KCD2RealAsyncTimewarpPlan.md` documents the earlier
  late-pose architecture for historical reference only.
- KCD2 camera acquisition uses a read-only `WHGame.dll` `CCamera::UpdateFrustumPlanes` hook in `framegen/reproj/Kcd2Camera.{h,cpp}`. KCD2 loads `WHGame.dll` after OptiScaler initializes, so installation retries lazily from reprojection packet capture once the module exists. It validates the gameplay camera through `CView` MSVC RTTI, then publishes Matrix34 pose/FOV via seqlock. The pose also carries the raw CCamera projection block (`+0x30..+0x7C`), live-validated on retail 1.5.6: near plane = float at `+0x54` (near-edge y, 0.05), far plane = float at `+0x6C` (far-edge y, 8000), projection-plane edge at `+0x5C` (`+0x60 = (1/tan(fov/2))·height/2`, x/z are raw half-extents), pixel aspect at `+0x40`; the ints at `+0x34/+0x38` are repurposed and must not be read as viewport dims. Near/far now feed `constants.cameraNear/Far` for validated depth math (RPD already corrected for D3D 0..1 depth + axial view-Z; the old OpenGL-style symmetric formula plus normalized-ray reconstruction was one cause of the earlier mode-1 double-warp). Depth mode is selectable again (configured mode honored with per-pixel rotation fallback for disocclusions); `PrepareRotationConstants(cb, /*inputLatched*/ true, yaw, pitch)` composes a late raw-mouse rotation as the warp target while the position lerp keeps rendered velocity; reconstruction always uses the rendered pose. Frustum callbacks are grouped into render-frame bursts: exact duplicates never advance history, but a changed pose or a >=8 ms gap does, so a stationary camera publishes a zero-velocity pair instead of extrapolating the last turn forever. Optional EMA smoothing (`[Reproj] Smoothing=0..0.95`, 0=off) low-passes the camera angular velocity to counter jitter at the cost of a few ms lag. Warp step is clamped absolutely by `ReprojMaxTimeStep`; there is no per-slot velocity limiter. Signatures/layout derive from MIT-licensed KCD2Tools/TPVCamera. Unknown builds must fail closed; never accept every frustum camera because the function also receives shadows/reflections/portals.
  Depth warp on KCD2: the projection block (near @+0x54, far @+0x6C, vfov, pixel aspect) is live-validated and feeds `constants.cameraNear/Far`, so the **configured** warp mode is honored when the constants pass the range checks (`CaptureFramePacket` no longer force-clamps KCD2 to mode 2). `RotationOnly=false` + `Mode=1` selects the depth-aware path with per-pixel rotation-homography fallback for disocclusions; an unknown build whose near/far fail the live validation still fails closed to the rotation homography. Do not regress this: the depth path stays opt-in per game until footage-tuned (disocclusion confidence/HUD epsilon still need real-footage validation).
...
- The 2026-08-23 display-clock revision supersedes the earlier real-anchor-plus-warp cadence: publication only supplies anchors, the worker selects the newest completed anchor at a refresh slot, every valid visible output is warped, and it issues exactly one `Present(1, 0)` per slot. Validation focus is now KCD2 async; DRG is only a regression comparison.
- The input-predicted timewarp machinery (`ReprojInputPredictor`, KCD2 phase model, `InputPredictorResponse`) was
  removed 2026-08-31 for the bare-basics baseline. Per-slot steering is the raw-mouse late latch only: at warp dispatch
  (and again ~0.75 ms before the present deadline when `ReprojLateLatchFence` is on) `ApplyLateInput` reads the fresh
  raw-motion totals (`OptiInput::GetRawMouseMotion`, WM_INPUT accumulation) since the packet capture, converts them with
  `MouseSensitivityX/Y` (radians/count; 0 auto-tracks from rendered pose pairs via `UpdateMouseSensitivity`), and
  composes that rotation via `PrepareRotationConstants(cb, true, yaw, pitch)` in place of velocity extrapolation. When
  the late latch is off or has no input, warps fall back to rendered-velocity extrapolation. CRITICAL historical note
  ...
  **COMPUTE-queue warp path (updated 2026-09-02, awaiting validation):** the async warp runs on a dedicated
  ...
  DIRECT queue, so the warp + backbuffer copy should reach GPU start within the slot. KCD2's premultiplied or
  straight-alpha isolated UI is sampled at exact pixel coordinates and composited into the warp output by RPD before
  the single backbuffer copy. The presenter waits for that compute fence on the CPU before `Present(1,0)`; it submits
  no HUD work to KCD2's contended DIRECT queue. `SubmitComputeCommandList`
  ...
  queue's — needed for correct GPU timing) tears the whole compute subsystem down and falls back to the DIRECT present
  queue. Validate cadence, `late=applied/sampled`, `maxDeg`, and `hud` from the once-per-second log summary plus live
  image/latency feel; these counters intentionally replace the removed heavyweight per-slot telemetry.
...
- Deliberately removed from the build (2026-08-31): `HybridFsrGenerator` (+ the FSR generated-content sequence,
  `FGOutput::HybridTimewarp` menu entry), `Kcd2Input` (passive dispatcher hook), `ReprojInputPredictor`, and
  `TargetPoseResolver`, plus `[Reproj] InputPredictor/Response/TargetPoseResolver/TargetPoseShadow/
  DispatchLeadOverrideMs/AdaptiveQueueLead` and `[HybridTimewarp] GeneratedFrames`. Do not re-add them while the
  bare-basics timewarp experiment is running.
...
- Rotation-warp performance pass (2026-09-03): `PrepareRotationConstants` CPU-bakes the complete output-pixel -> source-UV homography into the shader-private `prevCameraRight/Up/Forward` rows. RPD must consume those rows directly; do not restore per-pixel FOV/camera-ray reconstruction. Its interior fast path intentionally avoids the original-frame load, fetching it only in the edge-feather/disocclusion branch. `RP_Dx12` is a rotation-only 2-SRV pipeline (world + isolated UI); legacy velocity/depth descriptors and barriers were removed.
- Confirmed capture serialization bottleneck (2026-09-03): compute capture currently creates a game -> compute -> game dependency for every anchor (`_computeQueue->Wait(_uiFence)`, capture copies, then `_gameCommandQueue->Wait(_captureFence)`) while warp uses the same compute FIFO. This locksteps source/display near 90 FPS even with zero camera rotation. Fixed per `plans/async_capture_queue_decoupling.md` (v10.0.1-pre19): capture copies run on the game DIRECT queue in presentation order with no per-anchor game pin and no compute-capture subsystem; the presenter claims only the newest anchor whose `_uiFence` completion value is already reached and otherwise re-warps its active anchor (`capWait` counts those reuse slots, `capC` counts DIRECT captures).
...
The once-per-second runtime line must retain source/display cadence, misses, late-input applied/sampled, maximum late
rotation, and HUD-composite count because those values directly prove the requested behavior.
