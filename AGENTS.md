...
| `[AsyncTimewarp] DepthWarp` | 0 | Depth-corrected translation warp: `1` = blend with rotation fallback, `2` = visualize confidence (green) vs fallback (magenta); `0` forces rotation-only |
...
- v10.0.1-pre25 (2026-09-03): depth v2 — continuous confidence blend (residual 12→24 px, discontinuity 0.25→0.5) instead of the hard switch, plus `DepthWarp=2` confidence visualization (HUD skipped in viz). Default back to `0` (rotation) until footage validates; inversion exonerated (flag comes from KCD2's own FSR2 init). Validate via viz first (where is it green/red?), then blended feel on hills.
