...
- DirectComposition is experimental and borderless/DWM-dependent. On Proton, `CreateSwapChainForComposition` commonly returns `E_NOTIMPL`; async reprojection then uses an input-transparent child HWND with its own DXGI swapchain. Creation/runtime failure still falls back synchronously; keep `ReprojAsync=false` as the shipped default until hardware testing is complete.
