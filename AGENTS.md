...
- Async-timewarp-on-KCD2 root cause (pinned 2026-08-22): async creation + virtualization succeed on KCD2, but Proton/wine's d3d12core crashes when the game's habitual startup `ResizeBuffers` runs on a swapchain carrying `FRAME_LATENCY_WAITABLE_OBJECT` (same `d3d12core!00387c7d` MFA every time). Fix: `hkResizeBuffers`/`hkResizeBuffers1` skip geometry-identical resizes on an async reproj swapchain and return S_OK. Genuine resizes (resolution/format/count changes) still pass through and are expected to crash under wine — known limitation until identified further.
