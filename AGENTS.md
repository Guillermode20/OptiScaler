...
- Main-swapchain virtualization requires flip model plus a frame-latency waitable object and remains experimental on Windows and Proton. Keep `ReprojAsync=false` as the shipped default until the new path is validated; Deep Rock Galactic on Proton is validated only on the synchronous fallback.
