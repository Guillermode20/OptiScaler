...
### Anchor-swap smoothing (latency pass, v10.0.1.37+)

New-anchor slots lerp the held previous anchor's warped color into the warp (`PrevColor` t3 SRV, blend factor = `Strength`, hardcoded `kSwapBlendFactor = 0.25`) so the 60 Hz content swap does not snap; repeat slots are pure. The blend needs the previous anchor alive one extra slot, which the same hold provides for the UI borrow — `_heldPacketIndex` stays `Presenting` until the NEXT new-anchor switch or loop exit, never per-slot (early exits must retire it). The blend only runs on warp paths; the unwarped blit and sync fallbacks are unchanged.
