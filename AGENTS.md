...
- The game thread never waits for the GPU on the capture path: packet exhaustion AND a busy capture allocator both drop the anchor (signal, submit, advance, count `dropAnchor`) instead of stalling. Virtual-buffer handoff remains bounded (up to 5 s) because returning before `AdvanceReprojectionBuffer` completes would let the game render into a resource still read by capture. A skipped anchor is invisible; a stalled game thread deepens every hitch behind it.
