...
`[AsyncTimewarp] Enabled`, `TargetRefresh`, `SourceFramerateLimit`, `MouseSensitivityX/Y`, `Smoothing`, `LateSampleLead`, `HudIsolation`, plus the opt-in A/B experiments below (`EdgeExtensionPx`, `GuardCropPercent`, `Depth*`, `ContinuityLatch`) are the live keys (`Config.h` + `Config.cpp` parse/save them). All are exposed as live controls in the in-game menu's **Output (Async Timewarp)** section (`menu_common.cpp` `RenderFrameGenerationRuntimeSettings` — `Enabled` is the section's master checkbox) next to the poseAge/capWait readout. Bind new menu controls only to keys whose machinery exists on this branch — the `#if 0` block in `menu_common.cpp` holds dead toggles for removed machinery and stays compiled out. The A/B keys below are **off by default and unvalidated** — they flip the *shipped* live-validated rotation-only behavior and must win a KCD2 A/B before their defaults change.
...
| `[AsyncTimewarp] EdgeExtensionPx` | 0 (= legacy static edge) | A/B edge-fill: `>0` replaces the static unwarped disocclusion fallback with a clamped motion-coherent extension (boundary stretch + small inward blur, limit in px, soft fade back to the static anchor near the limit). Uncovered pixels move with the warp instead of freezing |
| `[AsyncTimewarp] GuardCropPercent` | 0 (= off) | A/B fixed guard margin (% of the image per side, `0..3`). The warp samples through a fixed per-axis inset window, so the outer source ring is genuine reserve the rotation reveals before the captured edge is exhausted. Costs a small permanent zoom/FOV shrink. HUD (isolated) is composited unwarped and does not zoom |
| `[AsyncTimewarp] DepthEnabled` | false | A/B depth translation residual v1 (KCD2 only). Rotation stays canonical; depth only adds a small per-pixel parallax residual (see below). Requires the tracked depth resource (KCD2 probe: R24G8_TYPELESS, reversed-Z, internal render res) + fresh CView pose |
| `[AsyncTimewarp] DepthInverted` | true | Reversed-Z depth semantics (the KCD2 probe). `false` = standard depth |
| `[AsyncTimewarp] DepthMaxResidualPx` | 10 | Per-pixel residual magnitude ceiling (`1..48`) — pixels needing more correction fade back to pure rotation |
| `[AsyncTimewarp] DepthVerticalScale` | 0.25 | Share of vertical/world-Z camera translation applied to the residual (walking-bob damp, `0..1`). Horizontal is always 100% |
| `[AsyncTimewarp] ContinuityLatch` | false | A/B anchor-switch continuity latch: on a real anchor switch, compares where the previous anchor extrapolates to at the display deadline vs the new anchor's prediction and carries the clamped difference (yaw/pitch about the same axes as the late latch) onto the new anchor, decaying 100% → 40% → 0 over two display slots. Resets immediately on camera cuts; never carries across unwarped/reset anchors
...
`Reproj: source=… FPS display=… FPS (new=… repeat=…) missed=… interval=mean/p95ms latchLead=…ms poseAge=…ms queue=… late=applied/samples maxDeg=… dropAnchor=… capC=… capWait=… (async virtual swapchain|safe sync, block=…ms)` (exact format in `AReproj_Dx12.cpp` `LogMetricsIfDue`; pinning tests assert its pieces).
...
- Screen edges feather to the source frame by source coverage — never clamp-sample off-screen warp coordinates (edge smear). The warp is rotation-first; the only additions are the three opt-in A/B paths above, all defaulting to today's validated behavior: (1) motion-coherent edge extension replaces the static fallback only when `EdgeExtensionPx > 0`; (2) a fixed guard-crop window only when `GuardCropPercent > 0`; (3) a depth-assisted *residual* only when `DepthEnabled` is set AND a fresh KCD2 CView pose proves the translation — depth never replaces the rotation homography, it is magnitude-clamped (`DepthMaxResidualPx`), silhouette-gated (local depth-continuity confidence), and zeroed on stalls/cuts/tiny motion. Do not reintroduce the removed full positional reprojection.
...
- Warp mode is rotation-first; translation (walking/hills) is a known limitation: the warp assumes infinite depth, so near geometry parallaxes wrong on stale anchors. Depth-corrected warping was prototyped (pre24/25) and fully removed after regressing feel without a footage-tuning channel. The conservative residual v1 on this branch (`DepthEnabled`, off by default) is NOT that positional reprojection: the rotation homography stays canonical and depth only nudges the source UV by `f·τ/z` where `τ = R_anchor·(p_latest − p_anchor)` (world-Z damped by `DepthVerticalScale`), sampled from the tracked depth at the rotation UV, confidence-gated at silhouettes and clamped to `DepthMaxResidualPx`. It needs a fresh CView pose per slot (stalls/cuts disable it) — see `plans/walking_judder_options.md` before extending it.
...
Capture worker + dedicated COPY capture queue, mid-frame world fence, COMPUTE warp queue, UI borrow (`uiBorrow` / `_heldPacketIndex`), repeat-warp shed (`EvaluateRepeatWarpShed`, stall/cadence EMAs, `RepeatWarp` key), swap-smooth prev-color blend, adaptive late-sample/dispatch controllers, per-slot telemetry (`ReprojTelemetry v=1`, `ReprojPipe`, slot dumps), the whole `#if 0` experimental menu block, `HybridFsrGenerator` (+ `FGOutput::HybridTimewarp`), `Kcd2Input`, `ReprojInputPredictor`, `TargetPoseResolver`, velocity clamps, and the *full positional* depth/MV warp selection (prototyped pre24/25, removed pre26). Deliberate exceptions on this branch: the opt-in `[AsyncTimewarp] SourceFramerateLimit` source cap (default 0, live-validated v10.0.1-pre45), the live ATW menu controls bound to existing machinery, and the four unvalidated opt-in A/B keys above (`EdgeExtensionPx`, `GuardCropPercent`, `Depth*`, `ContinuityLatch`) — all default to the validated behavior and none adds a queue, worker, or per-slot fence.
...
- **Working model for residual walking judder** (pre21 measurements, still current): stale anchors + walking translation that rotation-only warping cannot represent + snaps on anchor update. The candidates from `plans/walking_judder_options.md` are now implemented as **opt-in, off-by-default A/B keys** (unvalidated — order of the KCD2 A/Bs per the branch's live-validation rule): Option B (anchor-switch continuity latch → `ContinuityLatch`), Option C (conservative depth residual v1 → `DepthEnabled`/`DepthInverted`/`DepthMaxResidualPx`/`DepthVerticalScale`), and the visible-artifact edge fixes from `plans/timewarp_artifacts.md` (`EdgeExtensionPx`, `GuardCropPercent`). Option A stays the 60 Hz `SourceFramerateLimit` A/B; Option D (capture freshness) is unchanged.
