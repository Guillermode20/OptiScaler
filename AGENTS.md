...
4. Warps are rotation-only camera timewarp on a dedicated normal-priority COMPUTE queue (VKD3D serializes DIRECT queues ~10–17 ms behind the game, so the warp must not run there), followed by a GPU-side cross-queue wait (`_presentQueue->Wait(_computeFence)`) instead of a CPU-blocking completion stall. The isolated HUD is composited unwarped in the same dispatch by RPD — exactly once per output. Unwarped blits stay on the DIRECT SC queue (copy + raster UI in one submit, no cross-queue round-trip).
