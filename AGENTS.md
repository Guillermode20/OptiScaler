...
- Kingdom Come: Deliverance II v1.5.6 on Proton crashes during startup when async virtual-swapchain reprojection initializes (the game reaches `ResizeBuffers`, then faults in `d3d12core`). Keep reprojection disabled there; test the synchronous presenter separately before revisiting virtualization.
