...
- KCD2 camera acquisition uses a read-only `WHGame.dll` `CCamera::UpdateFrustumPlanes` hook in `framegen/reproj/Kcd2Camera.{h,cpp}`. KCD2 loads `WHGame.dll` after OptiScaler initializes, so installation retries lazily from reprojection packet capture once the module exists. It validates the gameplay camera through `CView` MSVC RTTI, then publishes Matrix34 pose/FOV via seqlock. KCD2 poses currently force reprojection mode 2 (rotation-only): its depth exists but near/far are unavailable, and mode 1 caused severe camera+MV/depth double-warp artifacts. KCD2 async rotation warps use a VELOCITY cap of 1.5 source-frame units instead of a hard displacement clamp: a displacement clamp freezes the image mid-turn during source stalls and snaps on anchor arrival, while the per-slot growth limiter keeps rotation smooth at bounded speed. Do not enable KCD2 depth mode until projection conventions and near/far are verified. Signatures/layout derive from MIT-licensed KCD2Tools/TPVCamera. Unknown builds must fail closed; never accept every frustum camera because the function also receives shadows/reflections/portals.
