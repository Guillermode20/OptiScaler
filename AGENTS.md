...
4. Warps are rotation-only camera timewarp on a dedicated normal-priority COMPUTE queue (VKD3D serializes DIRECT queues ~10–17 ms behind the game, so the warp must not run there), followed by a single backbuffer copy. The isolated HUD is composited unwarped in the same dispatch by RPD — exactly once per output. With a captured anchor depth (`[AsyncTimewarp] DepthWarp`, default on), RPD instead depth-reprojects static geometry around the predicted pose — exact for planes and pure rotation, first-order otherwise — and falls back per pixel to the rotation homography on silhouettes, large residuals, or uncovered UVs.
...
| `[AsyncTimewarp] DepthWarp` | true | Depth-corrected translation warp (mode 1) with per-pixel rotation fallback; `false` forces rotation-only — the instant kill-switch if depth misbehaves |
...
- `PrepareRotationConstants` CPU-bakes the complete output-pixel → source-UV homography into the shader-private `prevCameraRight/Up/Forward` rows; RPD consumes those rows directly — do not restore per-pixel FOV/camera-ray reconstruction. Its interior fast path skips the original-frame load except in the edge-feather/disocclusion branch. For depth mode it additionally stores the predicted pose in `Target*` (position advances linearly by the pair delta — the interval cancels, so cadence cannot skew it); depth sampling itself stays in the shader (per-pixel, bilinear world + point depth).
...
- v10.0.1-pre24 (2026-09-03): depth warp v1 for walking/hill translation. Probe proved KCD2 depth ready at capture (1505×847 R24G8T, inverted flag live, near/far 0.05/8000 live, no MSAA). Packets capture depth; RPD mode 1 = one-step depth correction with rotation fallback (silhouette discontinuity 0.5, residual 24 px — starting points, footage-tune). Kill-switch: `[AsyncTimewarp] DepthWarp=false`. Validate: walking/hills translation error, disocclusion fill, no regression on slow pans.
