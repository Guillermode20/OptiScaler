...
- KCD2 camera acquisition uses a read-only `WHGame.dll` `CCamera::UpdateFrustumPlanes` hook in `framegen/reproj/Kcd2Camera.{h,cpp}`. It validates the gameplay camera through `CView` MSVC RTTI, then publishes Matrix34 pose/FOV via seqlock. Signatures/layout derive from MIT-licensed KCD2Tools/TPVCamera. Unknown builds must fail closed; never accept every frustum camera because the function also receives shadows/reflections/portals.
