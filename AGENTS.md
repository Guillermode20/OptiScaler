...
- Synchronous reprojection must preserve the game-requested swapchain buffer count in both creation and resize hooks; only async virtualization requires a third buffer. KCD2 v1.5.6 on Proton crashes during startup after its two-buffer swapchain is coerced to three buffers. This fix is not live-validated until the rebuilt artifact is installed with `Reproj.Async=false`.
