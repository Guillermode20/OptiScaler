...
`[AsyncTimewarp] Enabled`, `TargetRefresh`, `SourceFramerateLimit`, `MouseSensitivityX/Y`, `Smoothing`, `LateSampleLead`, and `HudIsolation` are the only live keys (`Config.h` + `Config.cpp` parse/save them). All are exposed as live controls in the in-game menu's **Output (Async Timewarp)** section (`menu_common.cpp` `RenderFrameGenerationRuntimeSettings` — `Enabled` is the section's master checkbox) next to the poseAge/capWait readout. Bind new menu controls only to keys whose machinery exists on this branch — the `#if 0` block in `menu_common.cpp` holds dead toggles for removed machinery and stays compiled out.
...
| `[AsyncTimewarp] SourceFramerateLimit` | 0 (uncapped) | OPT-IN source cap for the 60→120 A/B. Paces the game-present thread onto an absolute deadline grid (`FrameLimit::paceReprojectionSource`, `OptiScaler/misc/FrameLimit.cpp`) only on the running-virtualized publication paths, after the anchor handoff and outside `block=` metrics. Overshoots advance the grid (no 57–58 FPS drift); no generic-`FramerateLimit` fallback. Live-validated v10.0.1-pre45. Keep 0 unless A/B testing |
...
- Screen edges feather to the source frame by source coverage — never clamp-sample off-screen warp coordinates (edge smear). Warp is rotation-only; there is no depth/MV/translation warp on this branch (see `plans/walking_judder_options.md` before proposing one).
...
- **Working model for residual walking judder** (pre21 measurements, still current): stale anchors + walking translation that rotation-only warping cannot represent + snaps on anchor update. Candidates ranked in `plans/walking_judder_options.md` (Option A: measure with the new 60 Hz cap; B: anchor-switch pose smoothing; C: depth warp — gated on a footage-tuning channel; D: capture freshness). Visible-artifact candidates in `plans/timewarp_artifacts.md`.
