...
- Async reprojection is DISPLAY-anchored, not render-rate-anchored: the presenter fills every refresh slot after the newest real frame with a warp until a newer packet preempts it. Warp count is whatever the display needs (never bounded by `ReprojMaxWarpFrames`, which only bounds the synchronous fallback), and pose age never disables async warping - late latch owns `[anchor -> scanout]` rotation from continuously integrated raw input, so aiming feels identical at 15 FPS and 60 FPS (warping an old anchor is exactly ATW over a stalled renderer). Async pacing uses software refresh deadlines plus the DXGI frame-latency waitable object: Proton can signal the waitable whenever queue capacity is available (every ~3–4 ms with tearing), so it must NOT be treated as the vblank heartbeat or the presenter floods warps and freezes visible output. Sleep preemptibly until ~3 ms before the software slot, consume the waitable once, then late-latch/dispatch and present without a second wait. Waiting until the exact deadline starts GPU work at scanout time; waiting again inside Present can add a refresh of latency. The validated synchronous path blocks the game present thread while emitting `ReprojMaxWarpFrames`-bounded warps. With `ReprojAsync=true`, the wrapper exposes private virtual backbuffers to the game and a worker owns every present on the real main swapchain; unavailable capabilities fall back synchronously. For a 120 Hz target, `ReprojMaxWarpFrames=3` covers a 30 FPS source (three warps plus one real frame); the existing cadence formula requests fewer warps at higher real FPS.
