...
> **Display-clock update (2026-08-23):** Async virtual-swapchain presentation is display-owned. Rendering publishes anchors but never presents them directly; the worker selects the newest completed anchor at a refresh slot, late-latches it, and issues exactly one vblank-synchronized present. The first display of a new valid anchor is warped as well, preventing renderer-rate snap-back. Camera-basis-free mouse rotation uses a projection-space homography and motion-vector-grid calibration. This detaches mouse-look rotation, but not translation, world animation, recoil, head bob, or scripted camera motion, from renderer cadence.
