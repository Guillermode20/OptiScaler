...
struct alignas(256) RP_Constants
{
    ...
    float cameraVFov;
    float cameraAspect;
    // ---- Tail (append-only; MUST stay in lock-step with the cbuffer below,
    //      including the 16-byte float4 alignment) ----
    float edgeExtensionPx; // <=0 = legacy static edge fallback; >0 = motion-coherent extension limit (px)
    float guardCropPxX;    // per-side guard-crop inset px (0 = off)
    float guardCropPxY;
    float edgeBlendPx; // feather width at the source edge (px)
    uint32_t depthEnabled; // 1 = depth translation residual allowed on this slot
    uint32_t depthInverted; // 1 = reversed-Z depth buffer
    float txCameraSpace[4]; // anchor-camera-space translation residual (meters), .w unused
    float depthPlanes[4];   // (near, far, unused, unused)
    float focalPxX; // source-color focal length px
    float focalPxY;
    float maxResidualPx; // per-pixel residual magnitude ceiling (px)
    float verticalScale; // CPU damping applied to world-Z translation (informational)
};

static_assert(offsetof(RP_Constants, cameraPosition) == 64, "RP_Constants must match HLSL cbuffer packing");
static_assert(offsetof(RP_Constants, txCameraSpace) == 224, "RP_Constants tail must match HLSL cbuffer packing");
