...
- Third live run confirmed all viewport/scissor/OM calls originate in `WHGame.dll`, and correlated the primary 1706x960 viewport with MRT passes carrying 3-4 color descriptors plus depth, followed by a full reduced-resolution/post chain. However, `Util::WhoIsTheCaller` emitted only the module name (no RVA), and most game RTV descriptors were absent from the intentionally capture-gated HUD registry, so resource dimensions remained unresolved. The follow-up now logs raw WHGame-relative caller RVAs and directly traces creation of 1706x960 RTVs (resource, descriptor, format, flags) before HUD capture filtering. Use those RVAs to disassemble the render-extent setup and identify the allocator/view-family structure. A frame capture showing distinct geometry outside the nominal rect is still mandatory before any oversized-RT implementation.
