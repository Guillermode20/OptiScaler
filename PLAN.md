...
- Next live gate: KCD2 with `PredictiveProbe=true` and `ContentInterpolation=false`; correlate gameplay world/HUD viewport, scissor, command list, and RT dimensions. A frame capture showing distinct geometry outside the nominal rect is still mandatory before any oversized-RT implementation.
