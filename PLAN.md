...
Status (2026-09-08): first viewport diagnostic live run was safe but inconclusive.
- `ResTrack_dx12` hooks `RSSetViewports` (vtable 21) + `RSSetScissorRects` (22) alongside `OMSetRenderTargets` (46). Passive observers only: never edit args, call original unconditionally, no allocation/wait.
- The first run exhausted its shared 24-call quota during startup, before async activation. It observed a repeated world-labelled 2560x1080 viewport/scissor at y=180 inside the 2560x1440 output, but captured no HUD-phase viewport and therefore did not establish a safe redirect boundary. Camera hook and fixed +1 degree probe both validated at the expected RVAs. This is not captured-image proof and does not approve oversized rendering.
- Diagnostic revised: logging begins only while the RTTI/signature-validated gameplay camera is live; world and HUD have independent 24-call viewport/scissor quotas; each line includes command-list identity; Scaleform OM lines include bound RT width/height/format and are also gameplay/probe gated. Probe off remains a cheap Config branch, with no resource mutation.
...
- Second live run established the split: gameplay world passes predominantly use a 1706x960 dynamic-resolution viewport/scissor (plus expected half/quarter/pyramid and 1300x1300 auxiliary passes), while every traced Scaleform pass binds a 2560x1440 RGBA8 RT with a 2560x1440 viewport/scissor. This proves HUD extent can be kept nominal, but not yet which upstream render-view/allocation owner must grow the complete world graph. The diagnostic caused a short startup throughput disturbance while emitting its bounded burst; do not use that interval for cadence conclusions.
- Third live run confirmed all viewport/scissor/OM calls originate in `WHGame.dll`, and correlated the primary 1706x960 viewport with MRT passes carrying 3-4 color descriptors plus depth, followed by a full reduced-resolution/post chain. However, `Util::WhoIsTheCaller` emitted only the module name (no RVA), and most game RTV descriptors were absent from the intentionally capture-gated HUD registry, so resource dimensions remained unresolved.
...
- The next bounded probe is implemented but not yet live-validated: a retail-1.5.6 signature/RVA-gated observer at `0x7B07F0` records the game-owned resource dimensions at `+0x90/+0x92/+0x94`, the compact descriptor values copied toward `0x7AD47C`, and the caller stack. It never modifies the descriptor and logs only the first 96 records plus all primary 1706x960 records. The first artifact crashed immediately after the third observed call because the helper's caller relies on `RCX` surviving this leaf despite the normal volatile-register convention; the post-call logger clobbered it at `WHGame.dll+0x7ACEE4`. The observer is corrected so all diagnostics occur before the original call, which is now the final operation and preserves the retail register state. Use the corrected artifact to identify the resource-object author and correlate world colour/depth/MV/temporal siblings before considering any mutation. A frame capture showing distinct geometry outside the nominal rect remains mandatory.
