...
- Next reverse-engineering probe records the caller module/RVA for each bounded viewport/scissor call and bounded world-phase OM bindings with command-list, descriptor/resource identity, dimensions, format, and DSV handle. Use those RVAs to disassemble the 1706x960 render-extent setup and identify the allocator/view-family structure. A frame capture showing distinct geometry outside the nominal rect is still mandatory before any oversized-RT implementation.
