...
- The next bounded probe is implemented but not yet live-validated: a retail-1.5.6 signature/RVA-gated observer at `0x7B07F0` records the game-owned resource dimensions at `+0x90/+0x92/+0x94`, the compact descriptor copied toward `0x7AD47C`, and the caller stack. It never modifies the descriptor and logs only the first 96 records plus all primary 1706x960 records. Use this to identify the resource-object author and correlate world colour/depth/MV/temporal siblings before considering any mutation. A frame capture showing distinct geometry outside the nominal rect remains mandatory.
